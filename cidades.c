...
int getId(ControlCid cids,char *cid,int *id) {
	...
	if(cids ==NULL || cids->cidades ==NULL || cids->ligacoes ==NULL || cid ==NULL) {
		...
		printf("%d",n);
		if(n==1) {
			*id= ((Cidade)data[0])->ide;
			/*imprimeCid((Cidade)data[0])*/
			;
		}
	}
	return valor;
}
int loadCamiaocity(camioes frota,ControlCid cids,char *nome,char *matricula) {
	int valor =OK;
	if(cids==NULL || nome ==NULL || matricula ==NULL) {
		valor =NO_INI;
	} else {
		int nc,ncc;
		void **cidade;
...
int removerCidade(ControlCid cids,char *nome) {
	...
	if(cids ==NULL || cids->cidades==NULL || cids->ligacoes==NULL ||nome ==NULL ) {
		...
		if(getId(cids,nome,&orig)==OK ) {
			...
			remove_hash(&cids->cidades,nova);
			/*remove da hash*/
			removeEdge(&cids->ligacoes,orig);
			/*custo da origem para outras cidades*/
			removeLigs(&cids->ligacoes,orig);
			/*remove do graph cidade com ligacao a orig*/
		}
	}
	return valor;
}
...
int imprimeCliez(ControlCid cids,char *nome) {
	...
	if(cids ==NULL || cids->cidades==NULL || cids->ligacoes==NULL||nome ==NULL) {
		...
		if(n==1) {
			valor=print(((Cidade)data[0])->clientes);
		} else {
			valor=NO_CID;
		}
	}
...
int getCamiao(ControlCid cids,char *orig,camiao cam) {
	...
	if(cids ==NULL || cids->cidades==NULL || cids->ligacoes==NULL||orig ==NULL) {
		...
		if(n==1) {
			valor=get_head(((Cidade)data[0])->camioes,cam);
		} else {
			valor=NO_CID;
		}
	}
...
int InsertCamiaoCid(Cidade cid,camiao cam) {
	int valor=OK;
	if(cid ==NULL || cam ==NULL) {	   
		valor =NO_INI;
	} else {
       // printf("/%p--xx%pxx\\\n",(cid->camioes)->root,cam);
		insert_LL(cid->camioes,&(cid->camioes)->root,cam);
		//imprime_elems(cid->camioes);
		//printf("xxxx/%p\\\n",(cid->camioes)->root->data);

		cid->nacedidas++;
		/*actualiza o numero de vezes acedida*/
	}
}
int InsertClienteCid(Cidade cid,cliente cls) {
	int valor=OK;
	if(cid ==NULL || cls ==NULL) {
		valor =NO_INI;
	} else {
...
int getNome(ControlCid cids,int id,char *nome) {
	int valor =OK;
	if(cids ==NULL || cids->cidades==NULL ) {
		valor =NO_INI;
	} else {
		void **data;
		int n;
		...
		if(n==1) {
			strncpy(nome,((Cidade)data[0])->nome,strlen(((Cidade)data[0])->nome));
		} else {
			valor=NO_CID;
		}
	}
...
/*requer que os camioes ja teham sido inseridos ou feito load*/
int loadcity_trucks(ControlCid cids,camioes camis,char *path) {
	int valor=OK;
	char *buffer,*b;
	buffer=malloc(200*sizeof(char));
	b=buffer;
	FILE *f;
	char matricula[MAXS],local[MAXS];
	if(path==NULL) {
		valor =NO_FILE;
	} else {
		f=fopen(path,"r");
		...
		if(f ==NULL) {
			...
			while(fgets(buffer,200,f)!=NULL) {
				...
				loadCamiaocity(camis,cids,local,matricula);
				buffer=b;
